...
from .shared_rewards import Zbot6DofRewards, ZbotCommonRewards


class Zbot6DofToSnakeEnv(Zbot6DofRewards, ZbotCommonRewards, ZbotBaseEnv):
    ...
    def _termination_terms(self) -> tuple[torch.Tensor, ...]:
        foot_missing_contact = torch.logical_and(
            torch.any(self.feet_contact_forces < 1.0, dim=1),
            self.base_pos_w[:, 2] > 0.1,
        )
        if foot_missing_contact[0]:
            print(self.feet_contact_forces[0])
        return (
            self._base_contact_termination(1000.0),
            self.base_lin_vel_w[:, 0, 2] < -0.1,
            foot_missing_contact,
        )
